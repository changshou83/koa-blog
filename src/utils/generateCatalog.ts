...
export function generateCatalog (htmlText: string = '') {
  ...
  let existsId: number = 0; // 用于区别同名标题
  ...
  for (let i = 0; i < tempEle.childNodes.length; i++) {
    ...
    if (ele.tagName && tags.includes(ele.tagName.toLowerCase())) {
      ...
      while (cmpContainer.length > 0) {
        const topLevel = [...tags].reverse().indexOf(cmpContainer[cmpContainer.length - 1].tag);
...
export function catalogTrack() {
  ...
  const $catalog = document.querySelector(`.catalog-container[name="${anchorName}"]`);
