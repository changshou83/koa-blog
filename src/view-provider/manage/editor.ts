...
export const xssFilterOptions: IFilterXSSOptions = {
  ...
  onIgnoreTagAttr(tag, name, value) {
    // 处理文字前景背景色问题
    if (name.slice(0, 6) === 'style')
      return `${name}="${escapeAttrValue(value)}"`;
  }
};

const template = (key: string) => t(`pages.Manage.Editor.Confirm.${key}`);
export const clearConfirm = (editorContent: Ref<string>) =>
  Modal.confirm({
    ...
    onOk() {
      editorContent.value = '';
      useMessage().info({
        message: template('SuccessText.message'),
        description: template('SuccessText.description')
      });
    },
    onCancel() {
      useMessage().info({
        message: template('ErrorText.message'),
        description: template('ErrorText.description')
      });
    }
  });
